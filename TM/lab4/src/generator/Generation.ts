import { RuleList, Token, $, Rule } from "./Types.js";
import CustomGrammarListener from "./CustomGrammarListener.js";
...
function generateClassMethod(
  ...
  return (
    `  ${name}(${parameters
      .map(([name, type, value]) =>
        value ? `${name}: ${type} = ${value}` : `${name}: ${type}`
      )
      .join(", ")}) {` +
    "\n" +
    body +
...
function generateTokenMatch(
  ...
  if (type === "regexp") {
    ...
    lines.push(
      `${
        isNext ? " else if " : "    if "
      }(this.curRemainder.startsWith("${value}")) {`
    );
...
export function generateLexerFileContent(
  tokens: Token[],
  ignore: Token[]
): string {
  ...
  const constructorBody = fieldsList
    ...
    .join("");
  const constructor = generateClassMethod(
    "constructor",
    [["input", "string"]],
    constructorBody
  );
  ...
  const nextChar = generateClassMethod(
    "nextChar",
    [["l", "number"]],
    nextCharBody
  );
...
export function generateLexerFile(
  CustomGrammar: CustomGrammarListener,
  alternativePath: string = path
): void {
  const content = generateLexerFileContent(
    Object.values(CustomGrammar.tokens),
    CustomGrammar.ignore
  );
  writeFileSync(`${alternativePath}/Lexer.ts`, content);
}

export function generateTokenFileContent(tokens: Token[]): string {
  let content = "";
  const tokensDeclarationList = tokens
    .map(
      ({ name, value, type }) =>
        `  ${name} = '${type === "regexp" ? name : value}',\n`
    )
    .join("");
...
export function generateTokenFile(
  CustomGrammar: CustomGrammarListener,
  alternativePath: string = path
): void {
...
export function isLL1(rules: RuleList) {
  const FIRST = generateFIRST(rules),
    FOLLOW = generateFOLLOW(rules);

  let isLL1: boolean = true;
...
function first(
  ...
  if (rule.length === 0) {
    ...
    return unite(
      diff(FIRST[rule[0].value], new Set([EPSILON])),
      FIRST[rule[0].value].has(EPSILON)
...
export function generateFIRST(rules: RuleList) {
  const nonterminals: string[] = Object.keys(rules);
  const FIRST: { [nonterminal: string]: Set<string> } = {};
...
export function generateFOLLOW(rules: RuleList) {
  const FIRST = generateFIRST(rules);
  const nonterminals: string[] = Object.keys(rules);
  const FOLLOW: { [nonterminal: string]: Set<string> } = {};
  for (const nonterminal of nonterminals) {
    ...
    for (const nonterminal in rules) {
      for (const rule of rules[nonterminal].rules) {
        for (let i = 0; i < rule.length; ++i) {
          ...
          FOLLOW[value] = unite(
            FOLLOW[value],
            diff(first(FIRST, rule.slice(i + 1)), new Set([EPSILON]))
          );
...
export function generateParserFileContent(ruleList: RuleList): string {
  const FIRST = generateFIRST(ruleList),
    FOLLOW = generateFOLLOW(ruleList);
  ...
  const consume = generateClassMethod(
    "consume",
    [["c", "string"]],
    consumeBody
  );
  ...
  for (const nonterminal in ruleList) {
    ...
    const parameters: [string, string][] = inheritedAttrs.map(
      ({ name, type }) => [`${name}`, `${type} | undefined`]
    );
    let body: any = [];
    if (synthesizedAttr) {
      body.push(
        `    let ${synthesizedAttr.name}: ${synthesizedAttr.type} | undefined;`
      );
    }
    body.push(
      ...[
    ...
    if (hasEPSILON) {
      body.push(
        ...[...FOLLOW[nonterminal]].map(
          (TOKEN) => `      case Token.${TOKEN}:`
        ),
      ...
      for (const { type, value, inheritedAttrs } of ruleEPSILON.slice(1)) {
        if (type === "code") {
          ...
          throw new Error(
            `EPSILON rule appears to have an entry of non-code type! Type: ${type}`
          );
        }
      }
...
export function generateTreeFileContent(
  treeFields: [string, string][]
): string {
...
export function generateTreeFile(
  CustomGrammar: CustomGrammarListener,
  alternativePath: string = path
): void {
  writeFileSync(
    `${alternativePath}/Tree.ts`,
    generateTreeFileContent(Object.entries(CustomGrammar.treeFields))
  );
}

export function generateUtilsFile(alternativePath: string = path): void {
  writeFileSync(
    `${alternativePath}/Utils.ts`,
    [
