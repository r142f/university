export default class CustomHaskellListener {
    constructor() {
        this.result = "";
        this.functions = {};
    }
        ...
        for (let key in this.functions) {
            ...
            for (let k = 0; k < implementations.length; ++k) {
                ...
                let argPattern = implementation.argPattern
                    .map((pattern, i) => pattern ? `${argNames[i]} == ${pattern}` : undefined)
                    .filter((v) => v)
                    .join(" && ");
                ...
                if (guard?.length > 0 || argPattern?.length > 0) {
                    ...
                    this.result += `${k === 0 ? "\n  if" : " else if"} (${ifCondition}) {${ifBody}}${k === implementations.length - 1 ? "\n" : ""}`;
                }
    ...
    exitFunction_implementation(ctx) {
        let name = ctx.NAME().text;
        let argNames = ctx
            .function_args_list()
...
//# sourceMappingURL=CustomHaskellListener.js.map